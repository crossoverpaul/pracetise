 ...
 *
 */
public class TestReptilesCNVD_OS implements Runnable{
	
	@Override
	public void run() {
		System.out.println("-------------操作系统----------------------------");
		//获取采集要用的参数
		String collectUrl ="http://www.cnvd.org.cn/flaw/typeResult?max=20&offset=0&typeId=27";
		String collectPage = "10";
		Set<String> urlSet = new HashSet<String>();
		try {
			for (int j = 0; j <= Integer.parseInt(collectPage); j++) {
				Connection conn = Jsoup.connect("http://www.cnvd.org.cn/flaw/typeResult?max=20&offset="+j*20+"&typeId=27");
						...
						if(!"".equals(elementsTr.text()) && null != elementsTr.text()){
							...
							while(iteratorTr.hasNext()){
								...
								try {
									...
									filePath =filePath+File.separator+"OS.txt";
