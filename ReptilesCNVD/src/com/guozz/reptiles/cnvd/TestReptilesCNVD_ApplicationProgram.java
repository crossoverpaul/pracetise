 ...
 *
 */
public class TestReptilesCNVD_ApplicationProgram implements Runnable{
	...
	public void run() {
		...
		//获取采集要用的参数
		String collectUrl ="http://www.cnvd.org.cn/flaw/typeResult?max=20&offset=0&typeId=28";
		String collectPage = "10";
		Set<String> urlSet = new HashSet<String>();
		try {
			for (int j = 0; j <= Integer.parseInt(collectPage); j++) {
				Connection conn = Jsoup.connect("http://www.cnvd.org.cn/flaw/typeResult?max=20&offset="+j*20+"&typeId=28");
						...
						if(!"".equals(elementsTr.text()) && null != elementsTr.text()){
							...
							while(iteratorTr.hasNext()){
								...
								try {
									...
									filePath =filePath+File.separator+"ApplicationProgram.txt";
